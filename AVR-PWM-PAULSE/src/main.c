...
//USART接收缓冲区
#define RX_BUFFER_SIZE_1 11                  //接收缓冲区大小，可根据需要修改。
unsigned char rx_buffer_1[RX_BUFFER_SIZE_1];   //定义接收缓冲区
unsigned char rx_counter_1 = 0;              //定义rx_counter_1为存放在队列中的已接收到字符个数。
...
{
    uchar status_1, data_1;
    status_1 = UCSR1A;
    data_1 = UDR1;
    if((flag1 & (1 << Usart1_RECVFlag1)) == 0) //判断是否允许接收一个新的数据包
    {
        if ((status_1 & (USART1_FRAMING_ERROR | USART1_PARITY_ERROR | USART1_DATA_OVERRUN)) == 0)
        {
            rx_buffer_1[rx_counter_1] = data_1;
            rx_counter_1++;
            switch (rx_counter_1)
            {
            case 1:       // 检验起始字符
            {
                if (data_1 != 0xf3) rx_counter_1 = 0;

            }
            break;
            case 2:
            {
                if (data_1 != 0xf4) rx_counter_1 = 0;
...
void TC1_compA_ISR(void)
{
    ...
    if((flag1 & (1 << Usart1_RECVFlag1)) != 0)
    {
        ...
        setTC1_frequency((rx_buffer_1[2] << 8) | rx_buffer_1[3]);
    }
}
/***************系统初始化函数 start ***********/
void init_cpu(void)
