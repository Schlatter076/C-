...
//USART接收缓冲区
#define RX_BUFFER_SIZE_1 30                  //接收缓冲区大小，可根据需要修改。
unsigned char rx_buffer_1[RX_BUFFER_SIZE_1];   //定义接收缓冲区
unsigned char rx_counter_1=0;                //定义rx_counter为存放在队列中的已接收到字符个数。 
...
#define TX_BUFFER_SIZE_1 17    
unsigned char tx_buffer_1[TX_BUFFER_SIZE_1];  
unsigned char tx_wr_index_1=0,tx_rd_index_1=0,tx_counter_1=0;

#pragma interrupt_handler usart1_txc_isr:33  //发送中断服务程序
void usart1_txc_isr(void)
{
    if (tx_counter_1)//队列不为空
    {
        --tx_counter_1;//出队列
        UDR1=tx_buffer_1[tx_rd_index_1];
        if (++tx_rd_index_1 == TX_BUFFER_SIZE_1) tx_rd_index_1=0;
    }
}
/***********USART1发送中断服务函数 end**********************/ 
...
{
    while (tx_counter_1 == TX_BUFFER_SIZE_1);
    CLI();//#asm("cli")关闭全局中断允许
    if (tx_counter_1 || ((UCSR1A & USART1_DATA_REGISTER_EMPTY)==0))//发送缓冲器不为空
    {
        tx_buffer_1[tx_wr_index_1]=c;//数据进入队列
        if (++tx_wr_index_1 == TX_BUFFER_SIZE_1) tx_wr_index_1=0;//队列已满
        ++tx_counter_1;
    }
    else
        UDR1=c;
...
void sendCMD_1(const uchar cmd[MY_SIZE]) 
{
	uint cmd_count = 0;
	for(cmd_count = 0; cmd_count < MY_SIZE; cmd_count++)
	{
		USART1_putchar(cmd[cmd_count]);
	}
}//*/
/************USART1发送字符数组函数end****************/
/************USART0接收字符数组函数start****************/
...
void recieveCMD_1(void) 
{
	...
	{
	    delay_nms(100);
		if((int)rx_buffer_1[n] == 0x01 && (int)rx_buffer_1[n+1] == 0x10 && (int)rx_buffer_1[n+2] == 0x00 && (int)rx_buffer_1[n+3] == 0x01 && (int)rx_buffer_1[n+4] == 0x00
		&& (int)rx_buffer_1[n+5] == 0x08 && (int)rx_buffer_1[n+6] == 0x10 && (int)rx_buffer_1[n+7] == 0x00 && (int)rx_buffer_1[n+8] == 0x60 && (int)rx_buffer_1[n+9] == 0x00
		&& (int)rx_buffer_1[n+10] == 0x80 && (int)rx_buffer_1[n+11] == 0x00 && (int)rx_buffer_1[n+12] == 0x76 && (int)rx_buffer_1[n+13] == 0x00 && (int)rx_buffer_1[n+14] == 0x67
		&& (int)rx_buffer_1[n+15] == 0x00 && (int)rx_buffer_1[n+16] == 0x81 && (int)rx_buffer_1[n+17] == 0x00 && (int)rx_buffer_1[n+18] == 0x68 && (int)rx_buffer_1[n+19] == 0x00
		&& (int)rx_buffer_1[n+20] == 0x66 && (int)rx_buffer_1[n+21] == 0x00 && (int)rx_buffer_1[n+22] == 0x62 && (int)rx_buffer_1[n+23] == 0x10 && (int)rx_buffer_1[n+24] == 0xb9)
		{
		    delay_nms(600);
			sendCMD_1(start);
			delay_nms(2000);
	...
	if(count < 4) 
	{
	    for(n = 0; n < RX_BUFFER_SIZE_1; n++)
	    {
	        USART1_putchar(rx_buffer_1[n]);
	    }
	    count++;
	}
...
{
    UCSR1B = 0x00; 
    UCSR1C = (1<<UCSZ11)|(1<<UCSZ10);        //(1<<URSEL)|异步，8位数据，无奇偶校验，一个停止位，无倍速 
    UBRR1L = BAUD_L;                        //设定波特率 
    UBRR1H = BAUD_H; 
    UCSR1A = 0x00; 
...
void init_cpu(void)
{
    ...
    DDRD  = 0xd8; //RXD1输入，且上拉有效  
	PORTD = 0x77;  //TXD1输出。
	DDRE =0x1e;  //RXD0输入，且上拉有效
	PORTE =0xe1;  //TXD0输出
